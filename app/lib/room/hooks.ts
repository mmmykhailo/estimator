 ...
 */
export function useEstimates(roomId: string): Round["estimates"] | null {
	const [estimates, setEstimates] = useState<Round["estimates"] | null>(null);
 ...
 */
export function useMyPeerId(_roomId: string): string {
 ...
 */
export function useConnectionStatus(_roomId: string): ConnectionStatus {
...
export function useWorkstreamSubmitters(
	...
	useEffect(() => {
		...
		Object.entries(estimates).forEach(
			([participantId, participantEstimate]) => {
				if (
 ...
 */
export function useAllEstimates(roomId: string): AllEstimatesItem[] {
	const completedRounds = useCompletedRounds(roomId);
	const [allEstimates, setAllEstimates] = useState<AllEstimatesItem[]>([]);

	useEffect(() => {
		const estimates: AllEstimatesItem[] = [];

		completedRounds.forEach((round) => {
			Object.entries(round.estimates).forEach(([, data]) => {
				estimates.push({
